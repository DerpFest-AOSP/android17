...
@Composable
fun NetworkSpeedStatusBarIcon(isDark: IsAreaDark, modifier: Modifier = Modifier) {
    val context = LocalContext.current
    val slot = context.getString(R.string.status_bar_network_speed)
    val controller = remember { NetworkSpeedController.get() }
    ...
    AndroidView(
        factory = { ctx -> NetworkSpeedView.fromContext(ctx, slot, blocked = false) },
        update = { view ->
