...
class PulseAudioDataProcessor(private val context: Context) {
    ...
    fun stopCapture() {
        ...
        try {
            ...
            Log.w(TAG, "stopCapture release", e)
        }
        visualizer = null
    ...
    private fun maybeRetargetVisualizer(configs: MutableList<AudioPlaybackConfiguration>) {
        ...
        try {
            ...
            Log.w(TAG, "release for retarget", e)
        }
        visualizer = null
        attachedSessionId = INVALID_SESSION
    ...
    /**
     * Picks the audio session that matches the **currently playing** local media app (same package
     * as [MediaSessionManager.getActiveSessions]), not merely the first entry in
     * [AudioManager.getActivePlaybackConfigurations] (which often favored one app, e.g. Spotify).
     */
    private fun pickSessionIdFromConfigs(
        ...
        if (targetPkg != null) {
            for (c in configs) {
                ...
                // Match the app that owns the active MediaSession (any usage — some players use
                // USAGE_UNKNOWN).
        ...
        // Music-like streams when we could not resolve a media session package
        for (c in configs) {
            val sid = c.sessionId
            if (sid > 0 && isLikelyMusicPlayback(c.audioAttributes)) return sid
        }

        // Last resort: any active session (legacy behavior)
        for (c in configs) {
            val sid = c.sessionId
            if (sid > 0) return sid
        }
    ...
    /**
     * Package name of the best candidate **local** controller that is actually playing, if any.
     */
