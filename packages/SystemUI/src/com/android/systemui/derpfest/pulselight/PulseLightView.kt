...
class PulseLightView @JvmOverloads constructor(
    ...
    private var currentAnimationEffect = EFFECT_NONE
    ...
    private fun setupContentObserver() {
        ...
        val pulseAmbientLightAnimationEffect = Settings.Secure.getUriFor(PULSE_AMBIENT_LIGHT_ANIMATION_EFFECT)
        ...
        val contentObserver = object: ContentObserver(Handler(Looper.getMainLooper())) {
            override fun onChange(selfChange: Boolean, uri: Uri?) {
                when (uri) {
                    ...
                    pulseAmbientLightAnimationEffect -> {
                        val newEffect = Settings.Secure.getStringForUser(
                            context.contentResolver,
                            PULSE_AMBIENT_LIGHT_ANIMATION_EFFECT,
                            UserHandle.USER_CURRENT
                        ) ?: EFFECT_NONE
                        if (currentAnimationEffect != newEffect) {
                            currentAnimationEffect = newEffect
                            stopEffectAnimation()
                            if (newEffect != EFFECT_NONE && isVisible) {
        ...
        context.contentResolver.registerContentObserver(
            pulseAmbientLightAnimationEffect, false, contentObserver, UserHandle.USER_CURRENT)
        ...
        contentObserver.onChange(true, pulseAmbientLightAnimationEffect)
    }
    ...
    companion object {
        ...
        private const val PULSE_AMBIENT_LIGHT_ANIMATION_EFFECT =
                Settings.Secure.PULSE_AMBIENT_LIGHT_ANIMATION_EFFECT
    }
}
