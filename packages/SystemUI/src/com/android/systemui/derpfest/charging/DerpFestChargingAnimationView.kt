...
package com.android.systemui.derpfest.charging
...
class DerpFestChargingAnimationView @JvmOverloads constructor(
    context: Context,
    ...
    private fun drawBatteryFill(canvas: Canvas) {
        ...
        val step = 2f
        while (x <= innerRight) {
            val waveY = fillTop + sin((x / batteryW * waveFreq * Math.PI + wavePhase).toDouble())
                .toFloat() * waveAmp
